...
있고 그 현재 위치는 §0.31에 있다 — (1)(2)(3) 완료, (4)(5)(6) 미수행. 남은 소유자
결정은 §4.2(5건),
착수 가능한 코드 작업은 §4.3이다. 이후 §1부터는 설계 목표와 08-17 이전 게이트·구현
...
5. **소유자 결정 5건** (§4.2 — §0.31이 출시 스위치와 과거 데이터 정책 2건을 추가 등재했다).
...
8. **출시 스위치 — `ready` 전환과 five-pin 승인의 절차** (신규, 2026-08-23, §0.31-⑥).
...
9. **과거 데이터 정책과 SC-02 기준** (신규, 2026-08-23, §0.31-⑦). 현재 발행분은 08-18·08-19

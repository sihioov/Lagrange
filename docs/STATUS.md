...
이 문서 커밋 기준 `main`은 `origin/main`보다 51커밋 앞서며 push하지 않았다. 소유자 파일
...
후속 실행에서 이 문서가 포함된 최종 clean HEAD의 image-build plan과 Docker Compose
preflight, immutable release dry-run, artifact operation plan은 모두 통과했다. 신규 manifest와
`/opt/lagrange/releases/<commit>` 대상은 존재하지 않아 덮어쓰기

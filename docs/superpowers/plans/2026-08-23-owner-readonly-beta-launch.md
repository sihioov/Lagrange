...
### 2.1 소유자 사전 승인 원칙과 현재 작업 경계 (2026-08-24)

소유자는 이 계획을 실행할 때 확정 범위 안의 세부 선택과 승인 게이트를 권장안대로
진행하는 원칙을 승인했고, 2026-08-24 최신 지시로 구현 시작을 명시했다. 위임 작업은
`paseo-delegate`를 통한 범위 고정 워커와 코디네이터 작업으로 분리하며, 소유자만 직접 할 수
있는 확인은 제거할 수 없을 때만 다음 검토 목록에 남긴다. 사전 승인 범위는 다음과 같다.

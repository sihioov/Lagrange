...
use result_model::manifest::{
    ArtifactManifest, ArtifactType, BacktestManifest, ManifestWriter, RunManifest,
};
...
async fn insert_test_job(pool: &PgPool, owner: Uuid) -> Result<Uuid, Box<dyn Error>> {
    let id: Uuid = sqlx::query_scalar(
        "INSERT INTO jobs (owner_user_id, job_type) VALUES ($1, 'backtest') RETURNING id",
    )
    .bind(owner)
    .fetch_one(pool)
    .await?;
...
async fn insert_test_strategy(pool: &PgPool) -> Result<(), Box<dyn Error>> {
...
fn sample_manifest(run_id: Uuid, owner: Uuid, job_id: Uuid, seed: u64) -> BacktestManifest {
    ...
    metrics.insert(
        ...
        domain::ReportedStat::from_f64(0.010_609_6).unwrap(),
    );
    metrics.insert(
        "max_drawdown".to_owned(),
        domain::ReportedStat::from_f64(0.0).unwrap(),
    );
    BacktestManifest {
        run: RunManifest {
            id: run_id,
...
async fn manifest_write_persists_rows_across_all_tables() {
    ...
    run_test("persist", &super_url, |super_url, db, pool| {
        Box::pin(async move {
            ...
            insert_test_strategy(pool).await?;
            let job: Uuid = insert_test_job(pool, owner).await?;
...
async fn manifest_write_is_idempotent() {
    ...
    run_test("idempotent", &super_url, |super_url, db, pool| {
        Box::pin(async move {
            ...
            insert_test_strategy(pool).await?;
            let job = insert_test_job(pool, owner).await?;
            let manifest = sample_manifest(Uuid::new_v4(), owner, job, 42);
            let worker_pool = connect_with_retry(&conn_url(super_url, "worker", db), 4).await?;
            let writer = ManifestWriter::new(worker_pool.clone());
            ...
            assert!(
                !second.inserted,
                "a retried job must not duplicate the manifest"
            );
            ...
            let artifacts: i64 = sqlx::query_scalar(
                "SELECT count(*) FROM result_artifacts WHERE backtest_run_id = $1",
            )
            .bind(manifest.run.id)
            .fetch_one(&worker_pool)
            .await?;
...
#[test]
fn manifest_write_rejects_non_finite_metric_at_the_json_boundary() {
    // Pure JSON-boundary test: no database needed, so it never depends on the
    // shared cluster's availability.
    ...
    assert!(
        err.is_err(),
        "1e999 metric must be rejected at the manifest boundary"
    );
}

#[tokio::test]
async fn manifest_write_rejects_malformed_artifacts_without_touching_the_db() {
    // The writer validates BEFORE the short transaction begins, so a malformed
    // artifact is rejected with a lazy (never-connected) pool.
    let super_url = match require_db_url() {
        Ok(u) => u,
        Err(_) => return,
    };
    ...
    let writer = ManifestWriter::new(lazy);
    let err = writer.write(&manifest).await;
    assert!(
        err.is_err(),
        "an invalid artifact sha256 must be rejected before insert"
    );
}

...
async fn http_contract_error_envelope_shape_and_codes() {
    ...
    let resp = h.get("/api/v1/strategies/does-not-exist", Some(&h.member)).await;
...
async fn http_contract_correlation_ids_propagate() {
    ...
    let req = axum::http::Request::builder()
        ...
        .header("cookie", format!("__Host-lagrange_session={}", h.member.cookie_value))
...
async fn http_contract_oversized_payload_is_typed_413() {
    ...
    let resp = h
        .post("/api/v1/strategies/buy_and_hold/configs", Some(&h.member), true, huge)
        .await;
...
async fn http_contract_strategy_catalog_happy() {
    ...
    assert!(!serialized.contains("owner_user_id"), "no tenant column leak");
    assert!(!serialized.contains("storage_path"), "no internal path leak");

...
fn mark(seq: u64, date: &str, prices: &[(&str, &str)]) -> LedgerEvent {
    LedgerEvent::MarkToMarket {
        ...
        prices: prices
            .iter()
            .map(|(id, px)| (instrument(id), price(px)))
            .collect(),
    }
}
...
fn deposit_sets_cash_and_deposits_accumulate() {
    ...
    apply_ok(
        &mut state,
        place(2, "00000000-0000-0000-0000-000000000001", A, Side::Buy, 10),
    );
    apply_ok(
        &mut state,
        fill(
    ...
    assert_eq!(
        state.cash(),
        krw("7397500.0000"),
        "5,000,000 - 10x10150 - 1000 fee + 2,500,000"
    );
}
...
fn buy_then_sell_round_trip_keeps_exact_fees() {
    ...
    apply_ok(
        &mut state,
        place(1, "00000000-0000-0000-0000-000000000001", A, Side::Buy, 100),
    );
    apply_ok(
        &mut state,
        fill(
    ...
    assert_eq!(
        state.position(&instrument(A)),
        None,
        "flat position after full sell"
    );
...
fn sell_without_position_is_rejected() {
    ...
    apply_ok(
        &mut state,
        place(1, "00000000-0000-0000-0000-000000000001", A, Side::Sell, 10),
    );
    let err = state
        .apply(fill(
            2,
...
fn sell_beyond_position_is_rejected() {
    ...
    apply_ok(
        &mut state,
        place(3, "00000000-0000-0000-0000-000000000002", A, Side::Sell, 60),
    );
    let err = state
        .apply(fill(
            4,
...
fn out_of_order_and_equal_seqs_are_rejected() {
    ...
    let err = state
        .apply(deposit(5, "1000000"))
        .expect_err("seq 5 == last seq 5");
    assert!(matches!(err, PortfolioError::OutOfOrderEvent { .. }));
}
...
fn impossible_precision_is_a_typed_error() {
    ...
    apply_ok(
        &mut state,
        place(1, "00000000-0000-0000-0000-000000000001", A, Side::Buy, 10),
    );
    let err = state
        .apply(fill(
            2,
...
fn zero_quantity_events_are_rejected() {
    ...
    apply_ok(
        &mut state,
        place(2, "00000000-0000-0000-0000-000000000002", A, Side::Buy, 10),
    );
    let err = state
        .apply(fill(
            3,
...
fn partial_fills_aggregate_and_equity_is_consistent() {
    ...
    assert_eq!(
        ...
        state
            ...
            .checked_add(
                &qty(100)
                    .checked_mul_price(&price("10200.0000"), Currency::KRW)
                    .expect("mark")
            )
            .expect("sum"),
    ...
    assert_eq!(
        state
            .equity_curve()
            .get(&TradingDate::parse("2026-02-02").expect("date")),
        Some(&krw("10001985.0000"))
    );
}
...
fn mark_requires_every_position_to_be_priced() {
    ...
    apply_ok(
        &mut state,
        place(3, "00000000-0000-0000-0000-000000000002", B, Side::Buy, 10),
    );
    apply_ok(
        &mut state,
        fill(
...
fn remarking_the_same_date_overwrites_deterministically() {
    ...
    assert_eq!(
        curve.get(&TradingDate::parse("2026-02-02").expect("date")),
        Some(&krw("9999500.0000"))
    );
}
...
fn minimum_fee_edge_is_deterministic_across_ledgers() {
    ...
    for s in [&mut s1, &mut s2] {
        apply_ok(
            s,
            place(1, "00000000-0000-0000-0000-000000000001", A, Side::Buy, 1),
        );
        apply_ok(
            s,
            fill(
    ...
    assert_eq!(
        f1.commission,
        krw("1000.0000"),
        "minimum commission applies exactly"
    );
    ...
    let debit = f1.cash_before.checked_sub(&f1.cash_after).expect("debit");
    let expected = f1
        .notional
...
fn full_state_json_round_trip_is_stable() {
    ...
    assert_eq!(
        back.canonical_bytes().expect("canonical bytes"),
        bytes,
        "byte-stable"
    );
    let hash = ContentHash::from_bytes(&bytes);
    assert!(
        hash.as_str().starts_with("sha256:"),
        "hashed canonical state"
    );
}
...
fn seeded_stream(seed: u64) -> (Money, Vec<LedgerEvent>, CostProfile) {
    ...
    for i in 0..6 {
        ...
        let buy = i < 3;
        let n0 = 1 + next() % 40;
        // The effective quantity must survive the branch (sells are capped
        // at half the held position: never short, never overfill).
        let n = if buy {
            held[idx] += n0;
            n0
        } else {
            let n = n0.min(held[idx] / 2);
            held[idx] -= n;
            n
        };
        if n == 0 {
        ...
        if buy {
            let notional = FixedPoint::from_i128(n0 as i128, 0)
                .expect("qty")
                .checked_mul(&px)
                .expect("notional");
            let raw_fee = notional
                .checked_mul(&rate)
                .expect("fee")
                .with_scale(4)
                .expect("fee scale");
            ...
            total_needed = total_needed
                .checked_add(&notional.checked_add(&fee).expect("sum"))
                .expect("total");
        }
    ...
    let initial = Money::from_fixed(
        total_needed.checked_add(&margin).expect("initial"),
        Currency::KRW,
    )
    ...
    for event in events {
        ...
        } = event
        {
            ...
            let delta = if side.is_buy() {
                cash = cash
                    .checked_sub(&notional.checked_add(&fee).expect("sum"))
                    .expect("debit");
                n as i64
            } else {
                cash = cash
                    .checked_add(&notional.checked_sub(&fee).expect("proceeds"))
                    .expect("credit");
                -(n as i64)
            };

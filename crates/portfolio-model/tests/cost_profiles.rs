...
fn krx_etf_default_is_versioned_and_deterministic() {
    ...
    assert_eq!(p.commission_rate, FixedPoint::parse("0.00015").expect("rate"));
    ...
    assert_eq!(p.rebalance_threshold, FixedPoint::parse("0.005").expect("threshold"));
    // Two builds are identical (no hidden randomness, no clock).
    assert_eq!(default_profile(), p, "default profile must be deterministic");
}
...
fn custom_profile_parses_explicit_settings() {
    ...
    assert_eq!(p.rebalance_threshold, FixedPoint::parse("0.01").expect("threshold"));
}
...
fn minimum_commission_applies_at_the_edge_deterministically() {
    ...
    let b = default_profile().estimate(Side::Buy, &qty(1), &price("1000")).expect("estimate");
    ...
    let b2 = default_profile().estimate(Side::Buy, &qty(3), &price("321")).expect("estimate");
    assert_eq!(b2.commission, krw("1000"));
    assert_eq!(b.commission, b2.commission, "minimum-fee edge must be deterministic");
}
...
fn commission_scales_above_the_minimum() {
    ...
    let b = default_profile().estimate(Side::Buy, &qty(10_000_000), &price("10000"))
        .expect("estimate");
...
fn sell_tax_only_on_sells() {
    let p = CostProfile::custom("0.001", "0", "0.002", 0, "1000", "0.001")
        .expect("custom profile");
    let buy = p.estimate(Side::Buy, &qty(100), &price("5000")).expect("estimate");
    assert_eq!(buy.tax, krw("0"), "no tax on buys");
    let sell = p.estimate(Side::Sell, &qty(100), &price("5000")).expect("estimate");
...
fn execution_price_embeds_slippage() {
    ...
    assert_eq!(p.execution_price(&raw, Side::Buy).expect("buy exec"), price("10160.1500"));
    assert_eq!(p.execution_price(&raw, Side::Sell).expect("sell exec"), price("10139.8500"));
    // Zero slippage: execution price is the raw open (Todo 10 execution basis).
    let p0 = CostProfile::custom("0.00015", "1000", "0", 0, "100000", "0.005")
        .expect("custom profile");
    assert_eq!(p0.execution_price(&raw, Side::Buy).expect("buy exec"), raw);
    assert_eq!(p0.execution_price(&raw, Side::Sell).expect("sell exec"), raw);
}
...
fn breakdown_identity_holds() {
    ...
    let b = default_profile().estimate(Side::Sell, &qty(50_000), &price("10150"))
        .expect("estimate");
...
fn breakdown_json_round_trip_is_stable() {
    ...
    assert_eq!(b, back);
    assert_eq!(bytes, serde_json::to_vec(&b).expect("reserialize"), "byte-stable JSON");
}
...
fn profile_json_round_trip_is_stable() {
    ...
    assert_eq!(p, back);
    assert_eq!(bytes, serde_json::to_vec(&p).expect("reserialize"), "byte-stable JSON");
}
...
fn higher_fees_never_produce_a_smaller_total_on_the_same_fill() {
    ...
    for (side, q, p) in cases {
        ...
        assert!(t_mid.amount() >= t_lo.amount(), "mid >= lo for {side:?} {q} x {p}");
        assert!(t_hi.amount() >= t_mid.amount(), "hi >= mid for {side:?} {q} x {p}");
    }
}

...
fn claims(iss: &str, sub: &str, email: &str, email_verified: bool, roles: Vec<&str>) -> IdTokenClaims {
    IdTokenClaims {
        iss: iss.to_string(),
        sub: sub.to_string(),
...
fn service() -> (InviteService, Arc<InMemoryInviteStore>, Arc<InMemoryUserStore>, Arc<InMemoryAuthAudit>, FakeClock) {
    ...
    let svc = InviteService::new(invites.clone(), users.clone(), Arc::new(clock), audit.clone());
...
async fn redemption_creates_user_and_consumes_invite() {
    ...
    svc.create_invite("user@example.com", Role::Member, 3600).await.unwrap();
    let identity = svc
        .resolve_identity(&claims("iss-1", "sub-1", "user@example.com", true, vec![]))
        .await
        .expect("redeemed");
    assert_eq!(identity.user_id.0, users.find_by_binding("iss-1", "sub-1").await.unwrap().unwrap().user_id.0);
    let invite = invites.find_by_email("user@example.com").await.unwrap().unwrap();
    assert_eq!(
        invite.redeemed_by,
...
async fn expired_invite_is_denied() {
    let (svc, _, _, _, _) = service();
    svc.create_invite("user@example.com", Role::Member, 10).await.unwrap();
    let err = svc
        .resolve_identity(&claims("iss-1", "sub-1", "user@example.com", true, vec![]))
        .await
...
async fn mismatched_email_is_denied() {
    let (svc, _, _, _, _) = service();
    svc.create_invite("invited@example.com", Role::Member, 3600).await.unwrap();
    let err = svc
...
async fn email_is_normalized_before_matching() {
    ...
    let user = users.find_by_binding("iss-1", "sub-1").await.unwrap().unwrap();
...
async fn binding_is_immutable_across_email_profile_change() {
    let (svc, _, users, _, _) = service();
    svc.create_invite("first@example.com", Role::Member, 3600).await.unwrap();
    let first = svc
        .resolve_identity(&claims("iss-1", "sub-1", "first@example.com", true, vec![]))
        .await
        .unwrap();
    let second = svc
        .resolve_identity(&claims("iss-1", "sub-1", "changed@example.com", true, vec![]))
        .await
        .expect("same (iss,sub) resolves to the same user");
    assert_eq!(first.user_id, second.user_id, "(iss,sub) must be the identity key");
...
async fn established_binding_needs_no_invite() {
    let (svc, _, users, _, _) = service();
    svc.create_invite("a@example.com", Role::Member, 3600).await.unwrap();
    svc.resolve_identity(&claims("iss-1", "sub-1", "a@example.com", true, vec![]))
        .await
        .unwrap();
    let user_before = users.find_by_binding("iss-1", "sub-1").await.unwrap().unwrap();
    let again = svc
...
async fn role_mapping_from_claims_and_invite_fallback() {
    let (svc, _, users, _, _) = service();
    svc.create_invite("owner@example.com", Role::Member, 3600).await.unwrap();
    let owner = svc
        .resolve_identity(&claims("iss-1", "sub-1", "owner@example.com", true, vec!["owner"]))
        .await
        .unwrap();
    assert_eq!(owner.role, Role::Owner);

    svc.create_invite("member@example.com", Role::Member, 3600).await.unwrap();
    let member = svc
        .resolve_identity(&claims("iss-1", "sub-2", "member@example.com", true, vec!["member"]))
        .await
        .unwrap();
    assert_eq!(member.role, Role::Member);

    svc.create_invite("fallback@example.com", Role::Owner, 3600).await.unwrap();
    let fallback = svc
        .resolve_identity(&claims("iss-1", "sub-3", "fallback@example.com", true, vec![]))
        .await
        .unwrap();
    assert_eq!(fallback.role, Role::Owner);

    assert_eq!(users.find_by_binding("iss-1", "sub-1").await.unwrap().unwrap().role, Role::Owner);
}

#[tokio::test]
async fn unknown_role_claim_is_fail_closed() {
    let (svc, _, _, _, _) = service();
    svc.create_invite("x@example.com", Role::Member, 3600).await.unwrap();
    let err = svc
        .resolve_identity(&claims("iss-1", "sub-1", "x@example.com", true, vec!["admin", "auditor"]))
        .await
...
async fn invalid_email_address_is_rejected() {
    let (svc, _, _, _, _) = service();
    assert!(svc.create_invite("not-an-email", Role::Member, 3600).await.is_err());
...
async fn user_id_is_never_the_email() {
    ...
    assert!(users.find_by_binding("iss-1", "sub-1").await.unwrap().is_some());
}

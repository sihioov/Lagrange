...
pub fn write_curated(dir: &Path, symbol: &str, year: i32, bars: &[FixtureBar]) {
    ...
    write_bars(&store.bars_path(MARKET, symbol, year, VERSION), &raw)
        .expect("write raw bars");
    let adjusted: Vec<AdjustmentBar> = bars.iter().map(|b| adjusted_row(symbol, b)).collect();
    write_adjusted_bars(&store.adjusted_bars_path(MARKET, symbol, year, VERSION), &adjusted)
...
pub fn raw_row(symbol: &str, b: &FixtureBar) -> CuratedBar {
    CuratedBar {
        ...
        batch_id: "batch-test".parse().expect("batch id"),
...
pub fn monthly_growth_bars(n: usize) -> Vec<FixtureBar> {
    ...
    const CLOSES: [&str; 12] = [
        ...
        "177.1561", "194.8717", "214.3589", "235.7948", "259.3742", "285.3117",
    ];
    let n = n.min(DATES.len());
    (0..n).map(|i| FixtureBar::new(DATES[i], CLOSES[i])).collect()
}

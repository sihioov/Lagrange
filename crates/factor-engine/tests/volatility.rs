...
fn ctx_for(dir: &std::path::Path, as_of: &str) -> FactorContext<'_> {
    let universe = FrozenUniverse::new("universe-test-1", &["VOL.KRX"]);
...
fn vol_20_constant_series_is_zero_and_null_before_window() {
    ...
    let f = RealizedVolFactor::new(20).expect("window").compute(&ctx).expect("compute");

    assert_eq!(f.rows.len(), n);
    for i in 0..20 {
        assert!(nth_value(&f, i).is_none(), "bar {i} NULL (fewer than 20 returns)");
    }
    for i in 20..n {
        assert_eq!(nth_value(&f, i), Some(0.0), "bar {i} vol of constant series = 0.0");
    }
}
...
fn vol_20_hand_calculated_reference() {
    ...
    let closes: Vec<i64> = (0..n).map(|i| 100 + (i % 7) as i64 * 3 + i / 7).collect();
    ...
    for i in 0..n {
        ...
        match (nth_value(&f, i), exp) {
            (None, None) => {}
            (Some(a), Some(e)) => {
                assert!((a - e).abs() <= 1e-9 * e.abs().max(1.0), "bar {i}: {a} vs {e}");
            }
...
fn vol_60_and_120_metadata() {
    let dir = tempdir().expect("temp");
    let n = 130;
    write_curated(dir.path(), "VOL.KRX", 2020, &closes_bars(n, |i| 100 + i as i64));
    let ctx = ctx_for(dir.path(), "2020-05-01");
    let f60 = RealizedVolFactor::new(60).expect("window").compute(&ctx).expect("compute");
    assert_eq!(f60.factor, "vol_60");
    assert!(nth_value(&f60, 59).is_none(), "bar 59 NULL (59 returns < 60)");
    assert!(nth_value(&f60, 60).is_some(), "bar 60 first non-null");
    let f120 = RealizedVolFactor::new(120).expect("window").compute(&ctx).expect("compute");
    assert_eq!(f120.factor, "vol_120");
    assert!(nth_value(&f120, 119).is_none(), "bar 119 NULL");
    assert!(RealizedVolFactor::new(10).is_err(), "unsupported window rejected");

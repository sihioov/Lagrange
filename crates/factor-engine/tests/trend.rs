...
fn trend_50_hand_calculated() {
    ...
    let t = TestCtx::new(dir.path(), &["TREND.KRX"], "2020-03-02");
    let f = TrendFactor::new(50)
        .expect("window")
        .compute(&t.ctx())
        .expect("compute");
...
fn trend_50_constant_series_is_zero() {
    ...
    for i in 49..60 {
        assert_eq!(
            nth_value(&f, i),
            Some(0.0),
            "bar {i} trend must be exactly 0.0"
        );
    }
}
...
fn trend_100_and_200_metadata_and_values() {
    ...
    write_curated(
        ...
        "TREND.KRX",
        2020,
        &consecutive_bars(210, |i| 100 + i as i64),
    );
    let t = TestCtx::new(dir.path(), &["TREND.KRX"], "2020-08-01");
    let f100 = TrendFactor::new(100)
        .expect("window")
        .compute(&t.ctx())
        .expect("compute");
    ...
    assert!(
        (a - expected_99).abs() < 1e-12,
        "trend_100 bar 99: {a} vs {expected_99}"
    );
    assert!(
        nth_value(&f100, 98).is_none(),
        "bar 98 NULL for 100-day window"
    );

    let f200 = TrendFactor::new(200)
        .expect("window")
        .compute(&t.ctx())
        .expect("compute");
    ...
    assert!(
        (a - expected_209).abs() < 1e-12,
        "trend_200 bar 209: {a} vs {expected_209}"
    );
    assert!(
        nth_value(&f200, 198).is_none(),
        "bar 198 NULL for 200-day window"
    );
...
fn trend_factor_metadata() {
    ...
    assert_eq!(
        f.lookback(),
        factor_engine::contract::Lookback::TradingDays {
            window: 50,
            min_periods: 50
        }
    );

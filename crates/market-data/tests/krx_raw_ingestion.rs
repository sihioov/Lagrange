...
use auth::entitlement::{
    Actor, CalendarDate, ContractRef, DataProvider, DatasetId, DocumentHash, Entitlement,
    EntitlementId, EntitlementService, EntitlementState, KrUseRegistry, UserId,
};
...
fn active_entitlement_service() -> EntitlementService {
    EntitlementService::new(vec![Entitlement::builder()
...
fn ingest_synthetic_bundle_creates_immutable_batch() {
    ...
    let dirs = store.batch_ids(PROVIDER_KRX, MARKET_KR, &req.date).expect("list");
    assert_eq!(dirs, vec![outcome.batch_id]);
    let manifest = store.read_manifest(PROVIDER_KRX, MARKET_KR).expect("manifest");
    assert_eq!(manifest.len(), 1);
    assert_eq!(manifest[0], outcome.entry);

    // Stored bytes are byte-identical to the recorded provider files.
    let back = store.read_batch_bytes(PROVIDER_KRX, MARKET_KR, &outcome.entry).expect("read back");
    for file in &back {
        let fixture = fs::read(
            std::path::Path::new(GOOD_BUNDLE).join(&file.file_name),
        )
        .expect("fixture readable");
        assert_eq!(file.bytes, fixture, "stored bytes must equal recorded provider bytes: {}", file.file_name);
    }
}
...
fn duplicate_delivery_two_batches_same_hash_first_untouched() {
    ...
    assert_ne!(first.batch_id, second.batch_id, "duplicate delivery must create a NEW batch");
    let h1: Vec<ContentHash> = first.entry.files.iter().map(|f| f.content_hash.clone()).collect();
    let h2: Vec<ContentHash> = second.entry.files.iter().map(|f| f.content_hash.clone()).collect();
    assert_eq!(h1, h2, "identical bytes must produce identical content hashes");

    // First batch untouched.
    let back1 = store.read_batch_bytes(PROVIDER_KRX, MARKET_KR, &first.entry).expect("first read");
    for file in &back1 {
        let fixture = fs::read(std::path::Path::new(GOOD_BUNDLE).join(&file.file_name)).expect("fixture");
        assert_eq!(file.bytes, fixture, "first batch must be untouched: {}", file.file_name);
    }
...
fn malformed_response_typed_failure_with_no_partial_output() {
    ...
    let dirs = store.batch_ids(PROVIDER_KRX, MARKET_KR, &req.date).expect("list");
    assert!(dirs.is_empty(), "malformed delivery must leave no batch dir: {dirs:?}");
...
fn timeout_typed_failure_with_no_partial_output() {
    ...
    assert!(
        matches!(err, IngestError::Provider(ProviderError::EndpointTimeout { kind: ResponseKind::Bars, .. })),
        "expected typed EndpointTimeout via Provider, got {err:?}"
    );
    assert!(store.batch_ids(PROVIDER_KRX, MARKET_KR, &req.date).expect("list").is_empty());
    assert!(store.read_manifest(PROVIDER_KRX, MARKET_KR).expect("manifest").is_empty());
}
...
fn path_traversal_typed_failure_with_no_partial_output() {
    ...
    assert!(
        matches!(err, IngestError::Provider(ProviderError::UnsafeFileName { .. })),
...
fn active_entitlement_is_referenced_on_the_manifest_row() {
    ...
    let manifest = store.read_manifest(PROVIDER_KRX, MARKET_KR).expect("manifest");
    assert_eq!(manifest[0].entitlement_reference, outcome.entry.entitlement_reference);
}
...
fn typed_error_plumbing_from_provider_and_store() {
    ...
    let provider_err: IngestError =
        ProviderError::EndpointTimeout { kind: ResponseKind::Bars, timeout_secs: 30 }.into();
    assert!(matches!(
        provider_err,
    ...
    .into();
    assert!(matches!(store_err, IngestError::Store(StoreError::FileExists { .. })));

    let unsupported: IngestError = ProviderError::UnsupportedKind(ResponseKind::Bars).into();
    assert!(matches!(unsupported, IngestError::Provider(ProviderError::UnsupportedKind(_))));
}

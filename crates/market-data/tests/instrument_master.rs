...
fn qa_resolve_069500_through_krx_and_kis() {
    // Manual QA channel: resolve 069500 through KRX and KIS aliases on an
    // effective date and print the canonical identity + metadata + the
    // timezone-aware open/close instants of the next KRX session 2020-02-03.
    let master = seed_universe();
    let date = d("2020-01-31");
    ...
    let instrument = master.instrument_on(&canonical, date).unwrap();
    let cal = market_data::calendar::krx_2020();

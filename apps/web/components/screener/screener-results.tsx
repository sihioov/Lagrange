...
export function ScreenerResults({
  ...
  const blocks = result.universes.map((universe) => ({
    universe,
    items: result.items.filter((item) => item.universe === universe),
  }));
  return (
    <section aria-labelledby="screener-results-title" className="data-report">
      <header className="report-heading">
        <div>
          ...
          <p>
            {result.items.length} instruments on this page, grouped by universe and ordered only
            within each immutable run.
          </p>
        </div>
      ...
      <div className="screener-universe-blocks">
        {blocks.map(({ universe, items }) => (
          <section
            aria-labelledby={`screener-${universe}-title`}
            className="screener-universe-block"
            key={universe}
          >
            <header className="panel-heading">
              <div>
                <h3 id={`screener-${universe}-title`}>{universeLabel(universe)}</h3>
                <p>{items.length} instruments in this universe block.</p>
              </div>
              <UniverseBadge universe={universe} />
            </header>
            {items.length === 0 ? (
              <p className="empty-copy">
                No eligible instruments satisfy this universe’s thresholds.
              </p>
            ) : (
              <div className="data-table-wrap">
                <table>
                  <caption>{universeLabel(universe)} candidate screener results</caption>
                  <thead>
                    <tr>
                    ...
                    {items.map((item) => (
                      <tr key={item.analysis_id}>
                        ...
                        <th scope="row">
                          <Link
                            className="data-link"
                            href={`/stocks/${encodeURIComponent(item.instrument_id)}?date=${encodeURIComponent(result.as_of)}&universe=${encodeURIComponent(item.universe)}`}
                          >

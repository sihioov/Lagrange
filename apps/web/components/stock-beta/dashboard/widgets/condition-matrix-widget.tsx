...
export function ConditionMatrixWidget({
  ...
  if (viewModel.data.rows.length === 0) {
    return (
      <WidgetFrame
        ...
        {visibleRows.map((row) => {
          ...
          return (
            <li key={row.instrument_id}>
              <button
                aria-label={`${row.instrument_name}, ${row.condition} · ${stockBetaConditionLabel(row.condition, t)}, ${t.scoreLabel} ${row.score}`}
              ...
              >
                <span>{row.rank}</span>
                <strong>{row.instrument_id.replace(".KRX", "")}</strong>

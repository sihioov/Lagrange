import type { LocaleDictionary } from "@/lib/i18n/locale";

export type StockBetaDictionary = {
  readonly activeFiltersHeading: string;
  readonly allMetricsLabel: string;
  readonly activityLabel: string;
  readonly activityDescription: string;
  readonly activityHeading: string;
  readonly activityPolicy: string;
  readonly activityProxyLabel: string;
  readonly approvedSnapshotLabel: string;
  readonly applyFilters: string;
  readonly asOfLabel: string;
  readonly artifactHashLabel: string;
  readonly audienceLabel: string;
  readonly backToWorkspace: string;
  readonly batchIdLabel: string;
  readonly bearishLabel: string;
  readonly bullishLabel: string;
  readonly capabilityLabel: string;
  readonly clearFilters: string;
  readonly conditionLabel: string;
  readonly conditionPolicy: string;
  readonly conditionDistributionLabel: string;
  readonly conditionReasonsDescription: string;
  readonly conditionReasonsHeading: string;
  readonly conditionSummaryDescription: string;
  readonly conditionSummaryHeading: string;
  readonly configuredResultsDescription: string;
  readonly configuredResultsLabel: string;
  readonly currentResultsDescription: string;
  readonly currentResultLeadersDescription: string;
  readonly currentResultLeadersHeading: string;
  readonly currentResultsLabel: string;
  readonly detailLinkLabel: string;
  readonly detailDescription: string;
  readonly detailEyebrow: string;
  readonly detailTitle: (instrument: string) => string;
  readonly drawdown120Label: string;
  readonly entitlementHashLabel: string;
  readonly factorLabel: string;
  readonly factorEvidenceCaption: string;
  readonly factorEvidenceDescription: string;
  readonly factorEvidenceHeading: string;
  readonly factorVersionLabel: string;
  readonly fixedListPolicy: string;
  readonly filteredSnapshotLabel: string;
  readonly filtersDescription: string;
  readonly filtersEyebrow: string;
  readonly filtersHeading: string;
  readonly filtersSummary: string;
  readonly genericUnavailableMessage: string;
  readonly genericUnavailableTitle: string;
  readonly indexMembershipLabel: string;
  readonly instrumentLabel: string;
  readonly instrumentHeaderDescription: string;
  readonly integrityMessage: string;
  readonly integrityTitle: string;
  readonly interpretationLabel: string;
  readonly invalidFiltersMessage: string;
  readonly invalidFiltersTitle: string;
  readonly instrumentNotFoundMessage: string;
  readonly instrumentNotFoundTitle: string;
  readonly latestDescription: string;
  readonly maxLabel: string;
  readonly materializationStatusLabel: string;
  readonly modeLabel: string;
  readonly minLabel: string;
  readonly noReasons: string;
  readonly noActiveFilters: string;
  readonly noResultsMessage: string;
  readonly noResultsTitle: string;
  readonly neutralLabel: string;
  readonly notReported: string;
  readonly originalPriceLabel: string;
  readonly originalPricePolicy: string;
  readonly provenanceDescription: string;
  readonly provenanceDisclosureLabel: string;
  readonly provenanceHeading: string;
  readonly publicationStatusLabel: string;
  readonly rawValueLabel: string;
  readonly rankLabel: string;
  readonly rankTableCaption: string;
  readonly rankTableDescription: string;
  readonly rankTableEyebrow: string;
  readonly rankTableHeading: string;
  readonly resultCountLabel: string;
  readonly registryHashLabel: string;
  readonly redistributionLabel: string;
  readonly registrationStatusLabel: string;
  readonly return120Label: string;
  readonly return20Label: string;
  readonly return60Label: string;
  readonly returnsDescription: string;
  readonly returnsHeading: string;
  readonly riskDescription: string;
  readonly riskHeading: string;
  readonly scoreLabel: string;
  readonly selectionBasisLabel: string;
  readonly signalMetricsHeading: string;
  readonly selectForPreview: string;
  readonly signalUnavailableMessage: string;
  readonly signalUnavailableTitle: string;
  readonly snapshotDescription: string;
  readonly snapshotHashLabel: string;
  readonly snapshotHeading: string;
  readonly snapshotStatusLabel: string;
  readonly sma20Label: string;
  readonly sma60Label: string;
  readonly strictPitLabel: string;
  readonly tableConditionLabel: string;
  readonly topFiveInTableLabel: string;
  readonly topFiveDescription: string;
  readonly topFiveEyebrow: string;
  readonly topFiveHeading: string;
  readonly trendLabel: string;
  readonly trendDownLabel: string;
  readonly trendUpLabel: string;
  readonly trendGroupHeading: string;
  readonly universeHashLabel: string;
  readonly valueLabel: string;
  readonly vendorSnapshotLabel: string;
  readonly volatility120Label: string;
  readonly volatility20Label: string;
  readonly volatility60Label: string;
  readonly volatilityGroupHeading: string;
  readonly volumeRatioLabel: string;
  readonly averageVolumeLabel: string;
  readonly warningLabel: string;
  readonly batchLabel: string;
  readonly betaLabel: string;
  readonly latestSnapshotLabel: string;
  readonly openDetailLabel: string;
  readonly searchLabel: string;
  readonly searchPlaceholder: string;
  readonly searchHint: string;
  readonly searchMatchesLabel: string;
  readonly noSearchResultsMessage: string;
  readonly removeFilterLabel: string;
  readonly columnViewLabel: string;
  readonly columnViewHeading: string;
  readonly signalProfileHeading: string;
  readonly signalProfileDescription: string;
  readonly signalDecompositionHeading: string;
  readonly signalDecompositionDescription: string;
  readonly conditionMatrixHeading: string;
  readonly conditionMatrixDescription: string;
  readonly snapshotTapeHeading: string;
  readonly snapshotTapeDescription: string;
  readonly currentSnapshotLabel: string;
  readonly returnsTabLabel: string;
  readonly volatilityTabLabel: string;
  readonly activityTabLabel: string;
  readonly zeroAxisLabel: string;
  readonly readOnlyBadgeLabel: string;
  readonly terminalContextLabel: string;
  readonly universeLabel: string;
  readonly policyBoundaryDescription: string;
  readonly policyBoundaryDetailsLabel: string;
  readonly policyBoundaryHeading: string;
  readonly policyBoundarySummary: string;
  readonly previewDescription: string;
  readonly previewEmptyMessage: string;
  readonly previewHeading: string;
  readonly previewSourceNote: string;
  readonly yes: string;
  readonly no: string;
  readonly pageDescription: string;
  readonly pageTitle: string;
  readonly policyAriaLabel: string;
};

export const stockBetaDictionary: LocaleDictionary<StockBetaDictionary> = {
  en: {
    activeFiltersHeading: "Active filters",
    allMetricsLabel: "All returned row metrics",
    activityLabel: "20-session trading-value activity proxy",
    activityDescription:
      "Volume and trading-value metrics are activity proxies returned by the API, not execution liquidity.",
    activityHeading: "Activity and volume",
    activityPolicy: "Volume and activity are an activity/liquidity proxy, not execution liquidity.",
    activityProxyLabel: "Activity proxy",
    approvedSnapshotLabel: "Approved snapshot",
    applyFilters: "Apply filters",
    asOfLabel: "As of",
    artifactHashLabel: "Artifact content hash",
    audienceLabel: "Audience",
    backToWorkspace: "Back to stock signal beta",
    batchIdLabel: "Batch ID",
    bearishLabel: "Bearish",
    bullishLabel: "Bullish",
    capabilityLabel: "Capability",
    clearFilters: "Clear filters",
    conditionLabel: "Scenario condition",
    conditionPolicy:
      "BULLISH / NEUTRAL / BEARISH are condition labels, not probabilities, target prices, buy/sell calls, weights, or orders.",
    conditionDistributionLabel: "Condition distribution",
    conditionReasonsDescription:
      "These reasons are returned by the API for the selected condition. They are evidence clauses, not forecasts.",
    conditionReasonsHeading: "Exact condition reasons",
    conditionSummaryDescription:
      "Counts are calculated from the rows in this response and are not a universe estimate.",
    conditionSummaryHeading: "Condition distribution",
    configuredResultsDescription:
      "The count and condition distribution describe the configured rows in this approved snapshot.",
    configuredResultsLabel: "Configured results",
    currentResultsDescription:
      "The count and condition distribution describe only the rows returned by the current filters.",
    currentResultLeadersDescription:
      "Up to five rows from these current results, in the order returned by the server.",
    currentResultLeadersHeading: "Current-result leaders",
    currentResultsLabel: "Current results",
    detailLinkLabel: "Open detail",
    detailDescription:
      "Inspect the price-and-volume factors, condition evidence, rank, and API provenance for one configured instrument.",
    detailEyebrow: "Instrument detail",
    detailTitle: (instrument) => `Stock signal beta · ${instrument}`,
    drawdown120Label: "120-session maximum drawdown",
    entitlementHashLabel: "Entitlement hash",
    factorLabel: "Factor",
    factorEvidenceCaption: "Factor explanations returned by the API for this instrument",
    factorEvidenceDescription:
      "Every factor name, value, and interpretation below is displayed from the selected detail response.",
    factorEvidenceHeading: "Factor evidence",
    factorVersionLabel: "Factor version",
    fixedListPolicy:
      "Owner-only configured fixed observation list — not current or historical index membership, and not the whole market.",
    filteredSnapshotLabel: "Filtered screen response",
    filtersDescription:
      "Filters are encoded in the URL and applied as a read-only query over the immutable ranked snapshot.",
    filtersEyebrow: "Read-only screen",
    filtersHeading: "Filter the ranked snapshot",
    filtersSummary: "Show filters",
    genericUnavailableMessage:
      "The stock signal beta could not be loaded. No signal rows are shown and no stale or synthetic fallback is substituted.",
    genericUnavailableTitle: "Stock signal beta unavailable",
    indexMembershipLabel: "Index membership",
    instrumentLabel: "Instrument",
    instrumentHeaderDescription:
      "An Owner-only price-and-volume research snapshot; rank, score, and condition remain server-supplied.",
    integrityMessage:
      "The approved signal snapshot failed its integrity check. No signal rows are shown and no fallback data is substituted.",
    integrityTitle: "Signal snapshot integrity failed",
    interpretationLabel: "Interpretation",
    invalidFiltersMessage:
      "Check the scenario, trend, and numeric range values, then submit the GET filter form again.",
    invalidFiltersTitle: "Stock signal filters are invalid",
    instrumentNotFoundMessage: "No approved signal row matches this configured instrument.",
    instrumentNotFoundTitle: "Instrument signal not found",
    latestDescription: "The latest approved read-only price-and-volume signal snapshot.",
    maxLabel: "Maximum",
    materializationStatusLabel: "Materialization status",
    modeLabel: "Mode",
    minLabel: "Minimum",
    noActiveFilters: "No filters applied.",
    noReasons: "No condition reason was reported by the API.",
    noResultsMessage: "No configured instruments match these filters.",
    noResultsTitle: "No signals match",
    neutralLabel: "Neutral",
    notReported: "Not reported",
    originalPriceLabel: "Original price",
    originalPricePolicy:
      "Original/unadjusted price data is used; corporate actions can distort returns and drawdowns.",
    provenanceDescription:
      "Only the provenance returned by the API is shown; this view does not imply publication beyond that record.",
    provenanceDisclosureLabel: "Show snapshot provenance",
    provenanceHeading: "API provenance",
    publicationStatusLabel: "Publication status",
    rawValueLabel: "API value",
    rankLabel: "Rank",
    rankTableCaption: "Ranked price-and-volume signal table",
    rankTableDescription:
      "The server-ranked rows remain in their supplied order, with every metric returned by the API.",
    rankTableEyebrow: "All configured instruments",
    rankTableHeading: "Full ranked table",
    resultCountLabel: "Result count",
    registryHashLabel: "Approval registry hash",
    redistributionLabel: "Redistribution",
    registrationStatusLabel: "Registration status",
    return120Label: "120-session return",
    return20Label: "20-session return",
    return60Label: "60-session return",
    returnsDescription:
      "Historical 20-, 60-, and 120-session returns from the signal snapshot; these observations are not forecasts.",
    returnsHeading: "Historical returns",
    riskDescription:
      "Historical volatility, maximum drawdown, and moving-average values from the signal snapshot.",
    riskHeading: "Volatility, drawdown, and trend",
    scoreLabel: "Score",
    selectionBasisLabel: "Selection basis",
    signalMetricsHeading: "Signal values",
    selectForPreview: "Select for preview",
    signalUnavailableMessage:
      "The approved signal snapshot is unavailable. No signal rows are shown and no fallback data is substituted.",
    signalUnavailableTitle: "Signal data unavailable",
    snapshotDescription:
      "As-of, result scope, and provenance status for the fetched signal response.",
    snapshotHashLabel: "Snapshot content hash",
    snapshotHeading: "Snapshot status",
    snapshotStatusLabel: "Response scope",
    sma20Label: "20-session moving average",
    sma60Label: "60-session moving average",
    strictPitLabel: "Strict PIT",
    tableConditionLabel: "Condition",
    topFiveInTableLabel: "Top 5 rows stay emphasized inside the full ranked table.",
    topFiveDescription: "The first five rows of the server-ranked snapshot, with rank preserved.",
    topFiveEyebrow: "Priority view",
    topFiveHeading: "Top 5",
    trendLabel: "Trend",
    trendDownLabel: "Trend down",
    trendUpLabel: "Trend up",
    trendGroupHeading: "Drawdown and moving averages",
    universeHashLabel: "Universe hash",
    valueLabel: "Value",
    vendorSnapshotLabel: "Vendor snapshot",
    volatility120Label: "120-session volatility",
    volatility20Label: "20-session volatility",
    volatility60Label: "60-session volatility",
    volatilityGroupHeading: "Volatility by lookback",
    volumeRatioLabel: "20/60 volume ratio",
    averageVolumeLabel: "20-session average volume",
    warningLabel: "Policy boundary",
    batchLabel: "Batch",
    betaLabel: "BETA",
    latestSnapshotLabel: "Latest response",
    openDetailLabel: "Open instrument detail",
    searchLabel: "Search instruments",
    searchPlaceholder: "Name or instrument ID",
    searchHint: "Only instruments in this response",
    searchMatchesLabel: "matches",
    noSearchResultsMessage: "No instruments in this response match the search.",
    removeFilterLabel: "Remove filter",
    columnViewLabel: "Columns",
    columnViewHeading: "Visible ranking columns",
    signalProfileHeading: "Signal profile",
    signalProfileDescription:
      "Compare exact response metrics by horizon; no price series or inferred scale is shown.",
    signalDecompositionHeading: "Signal decomposition",
    signalDecompositionDescription:
      "Raw score, condition, and row metrics from the current response for the selected instrument.",
    conditionMatrixHeading: "Condition matrix",
    conditionMatrixDescription:
      "Equal-area tiles preserve the server order and expose the current condition label.",
    snapshotTapeHeading: "Current snapshot tape",
    snapshotTapeDescription:
      "Current leaders from this response, shown in the order supplied by the server.",
    currentSnapshotLabel: "Current snapshot",
    returnsTabLabel: "Returns",
    volatilityTabLabel: "Volatility",
    activityTabLabel: "Activity",
    zeroAxisLabel: "Zero axis",
    readOnlyBadgeLabel: "READ ONLY",
    terminalContextLabel: "OWNER ONLY · READ ONLY · ORIGINAL PRICE",
    universeLabel: "Universe",
    policyBoundaryDescription:
      "Read-only price-and-volume evidence; expand the boundary for the complete policy notice.",
    policyBoundaryDetailsLabel: "Read the complete policy boundary",
    policyBoundaryHeading: "Research-only policy boundary",
    policyBoundarySummary: "Private Owner-only research surface; no orders or account actions.",
    previewDescription:
      "A compact view of the selected row, using only values already present in this response.",
    previewEmptyMessage: "Select a signal row to preview its returned metrics.",
    previewHeading: "Selected signal preview",
    previewSourceNote:
      "This preview does not fetch detail data; it uses the already-fetched row DTO.",
    yes: "Yes",
    no: "No",
    pageDescription:
      "A private Owner workspace for a fixed, read-only price-and-volume research beta.",
    pageTitle: "Stock signal beta",
    policyAriaLabel: "Stock signal beta policy boundary",
  },
  ko: {
    activeFiltersHeading: "적용 중인 필터",
    allMetricsLabel: "API가 반환한 모든 행 지표",
    activityLabel: "20거래일 거래대금 활동성 프록시",
    activityDescription:
      "거래량과 거래대금 지표는 API가 반환한 활동성 프록시이며 체결 유동성이 아닙니다.",
    activityHeading: "활동성과 거래량",
    activityPolicy: "거래량과 활동성은 활동·유동성 프록시일 뿐 체결 유동성을 뜻하지 않습니다.",
    activityProxyLabel: "활동성 프록시",
    approvedSnapshotLabel: "승인된 스냅샷",
    applyFilters: "필터 적용",
    asOfLabel: "기준일",
    artifactHashLabel: "아티팩트 콘텐츠 해시",
    audienceLabel: "대상",
    backToWorkspace: "종목 신호 베타로 돌아가기",
    batchIdLabel: "배치 ID",
    bearishLabel: "하락",
    bullishLabel: "상승",
    capabilityLabel: "기능",
    clearFilters: "필터 지우기",
    conditionLabel: "시나리오 조건",
    conditionPolicy:
      "BULLISH/NEUTRAL/BEARISH는 조건 레이블이며 확률, 목표가, 매수·매도 신호, 비중 또는 주문이 아닙니다.",
    conditionDistributionLabel: "조건 분포",
    conditionReasonsDescription:
      "선택한 조건에 대해 API가 반환한 사유입니다. 예측이 아닌 증거 절입니다.",
    conditionReasonsHeading: "정확한 조건 사유",
    conditionSummaryDescription:
      "이 응답의 행에서 계산한 수이며 전체 관찰 목록의 추정치가 아닙니다.",
    conditionSummaryHeading: "조건 분포",
    configuredResultsDescription: "수와 조건 분포는 승인된 스냅샷의 구성된 행을 설명합니다.",
    configuredResultsLabel: "구성 결과",
    currentResultsDescription: "수와 조건 분포는 현재 필터가 반환한 행만 설명합니다.",
    currentResultLeadersDescription: "현재 결과에서 서버가 반환한 순서대로 최대 5개 행입니다.",
    currentResultLeadersHeading: "현재 결과 선두 행",
    currentResultsLabel: "현재 결과",
    detailLinkLabel: "상세 열기",
    detailDescription:
      "구성된 한 종목의 가격·거래량 팩터, 조건 근거, 순위 및 API 출처를 확인하세요.",
    detailEyebrow: "종목 상세",
    detailTitle: (instrument) => `종목 신호 베타 · ${instrument}`,
    drawdown120Label: "120거래일 최대 낙폭",
    entitlementHashLabel: "권한 해시",
    factorLabel: "팩터",
    factorEvidenceCaption: "이 종목에 대해 API가 반환한 팩터 설명",
    factorEvidenceDescription:
      "아래 팩터 이름, 값 및 해석은 선택한 상세 응답에 있는 그대로 표시합니다.",
    factorEvidenceHeading: "팩터 근거",
    factorVersionLabel: "팩터 버전",
    fixedListPolicy:
      "오너 전용으로 구성된 고정 관찰 목록입니다. 현재 또는 과거 지수 편입 종목이나 전체 시장을 뜻하지 않습니다.",
    filteredSnapshotLabel: "필터링된 스크린 응답",
    filtersDescription:
      "필터는 URL에 인코딩되며 변경 불가능한 순위 스냅샷에 대한 읽기 전용 조회로 적용됩니다.",
    filtersEyebrow: "읽기 전용 스크리닝",
    filtersHeading: "순위 스냅샷 필터",
    filtersSummary: "필터 보기",
    genericUnavailableMessage:
      "종목 신호 베타를 불러오지 못했습니다. 신호 행은 표시하지 않으며 오래된 데이터나 합성 대체 데이터를 사용하지 않습니다.",
    genericUnavailableTitle: "종목 신호 베타를 사용할 수 없습니다",
    indexMembershipLabel: "지수 편입",
    instrumentLabel: "종목",
    instrumentHeaderDescription:
      "오너 전용 가격·거래량 연구 스냅샷이며 순위, 점수 및 조건은 서버가 제공한 값입니다.",
    integrityMessage:
      "승인된 신호 스냅샷의 무결성 검사가 실패했습니다. 신호 행은 표시하지 않으며 대체 데이터를 사용하지 않습니다.",
    integrityTitle: "신호 스냅샷 무결성 검사 실패",
    interpretationLabel: "해석",
    invalidFiltersMessage:
      "시나리오, 추세 및 숫자 범위를 확인한 후 GET 필터 양식을 다시 제출하세요.",
    invalidFiltersTitle: "종목 신호 필터가 올바르지 않습니다",
    instrumentNotFoundMessage: "구성된 종목과 일치하는 승인 신호 행이 없습니다.",
    instrumentNotFoundTitle: "종목 신호를 찾을 수 없습니다",
    latestDescription: "최신 승인 읽기 전용 가격·거래량 신호 스냅샷입니다.",
    maxLabel: "최대",
    materializationStatusLabel: "구체화 상태",
    modeLabel: "모드",
    minLabel: "최소",
    noActiveFilters: "적용된 필터가 없습니다.",
    noReasons: "API가 조건 사유를 보고하지 않았습니다.",
    noResultsMessage: "이 필터와 일치하는 구성 종목이 없습니다.",
    noResultsTitle: "일치하는 신호가 없습니다",
    neutralLabel: "중립",
    notReported: "보고되지 않음",
    originalPriceLabel: "원주가",
    originalPricePolicy:
      "원주가(비조정 가격)를 사용하며 기업행사로 수익률과 낙폭이 왜곡될 수 있습니다.",
    provenanceDescription:
      "API가 반환한 출처만 표시하며, 해당 기록을 넘어선 공개를 의미하지 않습니다.",
    provenanceDisclosureLabel: "스냅샷 출처 보기",
    provenanceHeading: "API 출처",
    publicationStatusLabel: "공개 상태",
    rawValueLabel: "API 값",
    rankLabel: "순위",
    rankTableCaption: "가격·거래량 신호 순위 표",
    rankTableDescription:
      "서버가 제공한 순위를 그대로 유지하며 API가 반환한 모든 지표를 표시합니다.",
    rankTableEyebrow: "전체 구성 종목",
    rankTableHeading: "전체 순위 표",
    resultCountLabel: "결과 수",
    registryHashLabel: "승인 레지스트리 해시",
    redistributionLabel: "재배포",
    registrationStatusLabel: "등록 상태",
    return120Label: "120거래일 수익률",
    return20Label: "20거래일 수익률",
    return60Label: "60거래일 수익률",
    returnsDescription: "신호 스냅샷의 과거 20·60·120거래일 수익률이며 관측값이지 예측이 아닙니다.",
    returnsHeading: "과거 수익률",
    riskDescription: "신호 스냅샷의 과거 변동성, 최대 낙폭 및 이동평균 값입니다.",
    riskHeading: "변동성·낙폭·추세",
    scoreLabel: "점수",
    selectionBasisLabel: "선정 기준",
    signalMetricsHeading: "신호 값",
    selectForPreview: "미리보기 선택",
    signalUnavailableMessage:
      "승인된 신호 스냅샷을 사용할 수 없습니다. 신호 행은 표시하지 않으며 대체 데이터를 사용하지 않습니다.",
    signalUnavailableTitle: "신호 데이터를 사용할 수 없습니다",
    snapshotDescription: "가져온 신호 응답의 기준일, 결과 범위 및 출처 상태입니다.",
    snapshotHashLabel: "스냅샷 콘텐츠 해시",
    snapshotHeading: "스냅샷 상태",
    snapshotStatusLabel: "응답 범위",
    sma20Label: "20거래일 이동평균",
    sma60Label: "60거래일 이동평균",
    strictPitLabel: "엄격한 PIT",
    tableConditionLabel: "조건",
    topFiveInTableLabel: "Top 5 행은 전체 순위 표 안에서 계속 강조됩니다.",
    topFiveDescription: "서버 순위를 보존한 스냅샷 첫 5개 행입니다.",
    topFiveEyebrow: "우선 보기",
    topFiveHeading: "Top 5",
    trendLabel: "추세",
    trendDownLabel: "하락 추세",
    trendUpLabel: "상승 추세",
    trendGroupHeading: "낙폭과 이동평균",
    universeHashLabel: "관찰 목록 해시",
    valueLabel: "값",
    vendorSnapshotLabel: "벤더 스냅샷",
    volatility120Label: "120거래일 변동성",
    volatility20Label: "20거래일 변동성",
    volatility60Label: "60거래일 변동성",
    volatilityGroupHeading: "기간별 변동성",
    volumeRatioLabel: "20/60 거래량 비율",
    averageVolumeLabel: "20거래일 평균 거래량",
    warningLabel: "정책 경계",
    batchLabel: "배치",
    betaLabel: "BETA",
    latestSnapshotLabel: "최신 응답",
    openDetailLabel: "종목 상세 열기",
    searchLabel: "종목 검색",
    searchPlaceholder: "이름 또는 종목 ID",
    searchHint: "현재 응답에 있는 종목만 검색",
    searchMatchesLabel: "개 일치",
    noSearchResultsMessage: "현재 응답에서 검색어와 일치하는 종목이 없습니다.",
    removeFilterLabel: "필터 제거",
    columnViewLabel: "열 보기",
    columnViewHeading: "순위 표에 표시할 열",
    signalProfileHeading: "신호 프로필",
    signalProfileDescription:
      "가격 시계열이나 추정 척도 없이 응답의 기간별 지표를 정확한 값으로 비교합니다.",
    signalDecompositionHeading: "신호 분해",
    signalDecompositionDescription: "선택한 종목에 대한 현재 응답의 원점수, 조건 및 행 지표입니다.",
    conditionMatrixHeading: "조건 매트릭스",
    conditionMatrixDescription: "같은 면적의 타일로 서버 순서와 현재 조건 레이블을 보존합니다.",
    snapshotTapeHeading: "현재 스냅샷 테이프",
    snapshotTapeDescription: "현재 응답의 선두 행을 서버가 반환한 순서대로 표시합니다.",
    currentSnapshotLabel: "현재 스냅샷",
    returnsTabLabel: "수익률",
    volatilityTabLabel: "변동성",
    activityTabLabel: "활동성",
    zeroAxisLabel: "0축",
    readOnlyBadgeLabel: "READ ONLY",
    terminalContextLabel: "오너 전용 · 읽기 전용 · 원주가",
    universeLabel: "관찰 목록",
    policyBoundaryDescription:
      "읽기 전용 가격·거래량 근거입니다. 전체 정책 안내는 경계를 펼쳐 확인하세요.",
    policyBoundaryDetailsLabel: "전체 정책 경계 읽기",
    policyBoundaryHeading: "연구 전용 정책 경계",
    policyBoundarySummary: "비공개 오너 전용 연구 화면이며 주문이나 계좌 작업은 없습니다.",
    previewDescription: "현재 응답에 이미 있는 값만 사용하는 선택 행의 간단한 보기입니다.",
    previewEmptyMessage: "반환된 지표를 미리 보려면 신호 행을 선택하세요.",
    previewHeading: "선택한 신호 미리보기",
    previewSourceNote:
      "이 미리보기는 상세 데이터를 다시 가져오지 않고 이미 가져온 행 DTO를 사용합니다.",
    yes: "예",
    no: "아니오",
    pageDescription:
      "고정된 읽기 전용 가격·거래량 연구 베타를 위한 비공개 오너 워크스페이스입니다.",
    pageTitle: "종목 신호 베타",
    policyAriaLabel: "종목 신호 베타 정책 경계",
  },
};

#!/usr/bin/env bash
# Build the ten owner-beta serving images without starting a container. The
...
The V2 manifest has one canonical record for each of the ten local serving
...
print_plan() {
  ...
  echo '  command: docker compose --env-file <env> --file <compose> build --pull=false <ten local services>'
...
inspect_built_images
write_manifest
echo "PRODUCTION_IMAGE_BUILD: PASS (ten images built and V2 manifest written: $manifest_file)"

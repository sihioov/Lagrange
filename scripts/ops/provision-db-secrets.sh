...
#
# The default mode is a non-mutating plan.  --apply is deliberately explicit
...
usage() {
  cat <<'EOF'
Usage: scripts/ops/provision-db-secrets.sh [--dry-run|--apply]
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --dry-run)
      [ "$mode_seen" -eq 0 ] || die 'choose exactly one mode: --dry-run or --apply'
      mode=dry-run
      mode_seen=1
      shift
      ;;
